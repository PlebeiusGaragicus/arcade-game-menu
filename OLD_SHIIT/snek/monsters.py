...
MONSTER_CHAR = 'M'
MONSTER_COLOR = term.white_on_black
...
monster = (random.randint(3, BORDER_WIDTH - 2), random.randint(3, BORDER_HEIGHT - 2))
...
with term.cbreak(), term.hidden_cursor():
    ...
    while True:
        ...
        print(term.move_xy(monster[0] * 2, monster[1]) + '  ')

        # Move the monster
        monster_dir = random.choice([(0, -1), (0, 1), (-1, 0), (1, 0)])
        monster = (monster[0] + monster_dir[0], monster[1] + monster_dir[1])

        # Check if the monster collided with the border
        if monster[0] in {2, BORDER_WIDTH + 1} or monster[1] in {2, BORDER_HEIGHT + 1}:
            monster_dir = (-monster_dir[0], -monster_dir[1])
            monster = (monster[0] + monster_dir[0], monster[1] + monster_dir[1])

        # Check if the monster collided with the snake
        if (monster[0], monster[1]) in snake:
            print(term.move_xy(term.width // 2, term.height // 2) + term.red_on_black('Game Over'))
            time.sleep(2)
            break

        # Draw the monster
        print(term.move_xy(monster[0] * 2, monster[1]) + MONSTER_COLOR(MONSTER_CHAR * 2))
        ...
        # Check if the snake collided with the monster
        if (x, y) == monster:
            print(term.move_xy(term.width // 2, term.height // 2) + term.red_on_black('Game Over'))
            time.sleep(2)
            break
